...
public class TimerSetSmooth<T> : WCSharp.Timers.TimerSet<T>
    where T : class, WCSharp.Timers.ISmoothAction
```
#### Type parameters
...
| [Add\(T\)](WCSharp.Timers.TimerSetSmooth_T_.Add(T).md 'WCSharp\.Timers\.TimerSetSmooth\<T\>\.Add\(T\)') | Will add the given action to this timer set, tracking it for execution whenever the timeout expires\. |
